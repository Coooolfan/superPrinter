<template>
    <div class="bg-gray-50 min-h-screen pb-20">
        <!-- 顶部导航栏 带返回按钮 -->
        <van-nav-bar title="确认订单" left-text="返回" left-arrow @click-left="goBack" fixed placeholder
        ...
        <div class="p-4">
            ...
            <div class="bg-white rounded-lg shadow-sm p-4 mb-4">
                ...
                <div v-if="requiredPaperCount" class="mt-4 p-3 bg-blue-50 rounded-lg">
                    ...
                    <div class="text-xs text-blue-600 mt-1" v-if="printSettings.duplex && documentPageCount !== null">
                        <van-icon name="info-o" class="mr-1" />
                        已按双面打印计算纸张用量 ({{ documentPageCount }}页÷2≈{{ Math.ceil(documentPageCount / 2) }}张/份×{{
            ...
            <div class="bg-white rounded-lg shadow-sm p-4 mb-4">
                ...
                <!-- 使用打印机选择器组件 -->
                <printer-selector v-model="selectedPrinter" :printers="printers" :loading="loadingPrinters"
                    :print-requirements="printRequirements" @search="onSearch" />
            </div>

            <!-- 备注信息 -->
            <div class="bg-white rounded-lg shadow-sm p-4 mb-4">
                <div class="text-lg font-medium mb-3">备注信息</div>
                <van-field v-model="remark" type="textarea" placeholder="请输入备注信息（可选）" rows="2" autosize />
            </div>
        </div>
        ...
        <div class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200">
            ...
            <div class="flex justify-between items-center px-4 py-2">
                <div class="flex flex-col">
                    ...
                    <!-- 参数不匹配提示 -->
                    <div v-if="selectedPrinter && !isPrinterMatchRequirements" class="text-xs text-orange-500 mt-1">
                        {{ getMismatchReason }}
                    </div>
                </div>
...
import { getOrderPreToken, type OrderPreBalanceResponse } from '@/api/order'
...
import { getOrderPreBalance, createOrder } from '@/api/order' // 导入新的 API 方法
...
const pollDocumentPageCount = async () => {
    ...
    try {
        ...
        if (typeof pageCount === 'number' && pageCount > 0) {
            documentPageCount.value = pageCount
            // 停止轮询
...
const totalPrice = computed(() => {
    ...
    // 特惠打印机 五折
    const printerDiscount = printer.printerId === 0 ? 0.5 : 1.0
...
async function createOrderHandle() {
    ...
    const resp = await createOrder({
        ...
        fileIds: orderFileStore.getOrderFile().join(','), // 上传的文件IDs
...
const printRequirements = computed(() => ({
    ...
    requiredPaperCount: requiredPaperCount.value ?? undefined // Convert null to undefined
}))
</script>
