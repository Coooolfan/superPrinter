...
public class RedisLockUtil {
    ...
    private static final DefaultRedisScript<String> CREATE_ORDER_LOCK_SCRIPT = new DefaultRedisScript<>();
     ...
     */
    public String createOrderLock(String token, String lockKey, String lockValue) {
        List<String> keys = List.of(token, lockKey);
        return redisTemplate.execute(CREATE_ORDER_LOCK_SCRIPT, keys, lockValue);
    }
     ...
     * @return 错误消息
     */
    public String getCreateOrderLockErrorMessage(long errorCode) {
        return switch ((int) errorCode) {
            case -1 -> "订单token无效或已过期";
            case -2 -> "文件页数计算尚未完成，请稍后再试";
            case -3 -> "操作过于频繁，请稍后再试";
            default -> "未知错误";
        };
    }
