...
public class MinioUtil {
     ...
     * @return 文件信息对象
     * @throws Exception 获取文件信息过程中可能发生的异常
     */
    public MinioObject getMinioObject(String bucket, String objectName) throws Exception {
        ...
        minioObject.setFilename(item.object());
        minioObject.setSize(item.size());
        minioObject.setDirectory(false);
        return minioObject;
    }
     ...
     */
    public List<MinioObject> listFiles(String bucket) throws RuntimeException {
        Iterable<Result<Item>> results = minioClient.listObjects(
                ListObjectsArgs.builder().bucket(bucket).recursive(true).build());

        List<MinioObject> infos = new ArrayList<>();
        results.forEach(r -> {
            MinioObject info = new MinioObject();
            try {
                Item item = r.get();
